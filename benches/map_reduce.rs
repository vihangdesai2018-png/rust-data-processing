...
use rust_data_processing::processing::{
    ReduceOp, VarianceKind, arg_max_row, feature_wise_mean_std, reduce, top_k_by_frequency,
};
...
fn bench_map_reduce(c: &mut Criterion) {
    ...
    for &n in &[20_000usize, 100_000] {
        ...
        group.bench_with_input(
            BenchmarkId::new("in_memory_filter_map_reduce_sum", n),
            &ds,
            |b, ds| {
                b.iter(|| {
                    ...
                    let mapped =
                        rust_data_processing::processing::map(black_box(&filtered), |row| {
...
fn bench_feature_wise(c: &mut Criterion) {
    ...
    for &n in &[50_000usize, 100_000] {
        ...
        group.bench_with_input(BenchmarkId::new("in_memory_one_pass", n), &ds, |b, ds| {
            b.iter(|| {
                black_box(
        ...
        group.bench_with_input(
            BenchmarkId::new("polars_single_collect", n),
            &ds,
            |b, ds| {
                b.iter(|| {
                    black_box(
...
fn bench_arg_topk(c: &mut Criterion) {
    ...
    group.bench_function("top_k_frequency_grp_k8", |b| {
        b.iter(|| {
            black_box(top_k_by_frequency(black_box(&ds), black_box("grp"), black_box(8)).unwrap())
        })
    });
...
fn bench_group_by_polars(c: &mut Criterion) {
    ...
    for &n in &[20_000usize, 100_000] {
        let ds = make_dataset_grouped(n);
        group.bench_with_input(
            BenchmarkId::new("mean_std_sum_count_distinct", n),
            &ds,
            |b, ds| {
                b.iter(|| {
                    black_box(
